...
I built a programming language where every number you print is inflated 10%. Print it again, it grows. Nobody has ever seen numbers like this.
...
New in TRUMPLANG: `IN TWO WEEKS`, a block that is scheduled and never runs.
...
Other languages: try / catch.

TRUMPLANG: `I ALONE CAN FIX IT` … `WITCH HUNT!`
...
`EXECUTIVE ORDER WINNING ABSOLUTELY LOSING` and suddenly addition subtracts. Your whole program, instantly.
...
`CHAPTER 11` wipes every variable in your program and keeps going. The brand survives.
...
`STOP THE COUNT` breaks out of a loop, but only while you're ahead.
...
TRUMPLANG style guide:

• Every keyword is UPPERCASE
• Every variable ends in !
...
I had the idea for a programming language based on his tweets back in 2016. Then I waited. In two weeks, every time.
