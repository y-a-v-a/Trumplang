...
CustomTrumplangVisitor.prototype.visitVariableDeclaration = function(ctx) {
  ...
  console.log("Declaring variable:", variableName, "with type:", dataType);
  
  // Verify variable naming convention (should end with !)
  if (!variableName.endsWith('!')) {
    throw new Error('THERE\'S NO ENTHUSIASM HERE, FOLKS! VARIABLES NEED ENERGY! BIG ENERGY!');
  }
  ...
  if (dealDeclCtx) {
    ...
    console.log("  with deal structure:", value);
...
CustomTrumplangVisitor.prototype.visitCommentStatement = function(ctx) {
  ...
  let comment = '';
  for (let i = 1; i < ctx.getChildCount(); i++) { // Skip the "A LOT OF PEOPLE ARE SAYING" part
    const child = ctx.getChild(i);
...
CustomTrumplangVisitor.prototype.visitAssignmentStatement = function(ctx) {
  ...
  console.log(`Assigning ${variableName} = ${value}`);
...
CustomTrumplangVisitor.prototype.visitIncrementStatement = function(ctx) {
  ...
  console.log("Incrementing variable:", variableName);
...
CustomTrumplangVisitor.prototype.visitDecrementStatement = function(ctx) {
  ...
  console.log("Decrementing variable:", variableName);
...
CustomTrumplangVisitor.prototype.visitFactor = function(ctx) {
  ...
  if (varToken) {
    ...
    console.log("Looking up variable:", varName);
...
CustomTrumplangVisitor.prototype.visitElseIfStatement = function(ctx) {
  ...
  if (conditionCtx) {
    const conditionResult = this.visit(conditionCtx);
    console.log("Else-if condition result:", conditionResult);
...
CustomTrumplangVisitor.prototype.visitElseStatement = function(ctx) {
  ...
  for (let i = 0; i < ctx.getChildCount(); i++) {
    ...
    if (child.ruleIndex === 1) { // statement rule index
      statements.push(child);
    }
  }
  
  console.log("Executing else block with", statements.length, "statements");
...
CustomTrumplangVisitor.prototype.visitForLoop = function(ctx) {
  ...
  console.log(`For loop with variable ${loopVar}`);
  ...
  console.log(`From ${startValue} to ${endValue}`);
  ...
  try {
    ...
    for (let i = startValue; i <= endValue; i++) {
      ...
      console.log(`For loop iteration: ${loopVar} = ${i}`);
...
CustomTrumplangVisitor.prototype.visitForEachLoop = function(ctx) {
  ...
  console.log(`ForEach loop with item ${itemVar} from array ${arrayVar}`);
  ...
  try {
    ...
    for (const item of array.value) {
      ...
      console.log(`ForEach iteration: ${itemVar} = ${item}`);
...
CustomTrumplangVisitor.prototype.visitFunctionDeclaration = function(ctx) {
  ...
  console.log(`Function declaration with ${ctx.getChildCount()} children`);
  
  // Helper to dump child info
  const dumpChild = (i, child) => {
    if (child) {
      const type = child.symbol ? `Token[${child.symbol.type}]:${child.getText()}` : 
                   child.ruleIndex !== undefined ? `Rule[${child.ruleIndex}]` : 'Unknown';
      console.log(`Child ${i}: ${type}`);
      
      // Extra debug for parameter list
      if (child.ruleIndex === 8) { // parameterList rule index
        console.log(`  Found parameterList with ${child.getChildCount()} children`);
        for (let j = 0; j < child.getChildCount(); j++) {
          const paramChild = child.getChild(j);
          const paramType = paramChild.symbol ? `Token[${paramChild.symbol.type}]:${paramChild.getText()}` : 
                            paramChild.ruleIndex !== undefined ? `Rule[${paramChild.ruleIndex}]` : 'Unknown';
          console.log(`  Param Child ${j}: ${paramType}`);
        }
      }
    }
  };
  ...
  for (let i = 0; i < ctx.getChildCount(); i++) {
    ...
    if (child && child.symbol && child.symbol.type === 60) { // IDENTIFIER token type
      identifier = child.getText();
      console.log(`Found function name: ${identifier}`);
    } else if (child.ruleIndex === 8) { // parameterList rule index
      paramListCtx = child;
      console.log(`Found parameter list`);
    } else if (child.ruleIndex === 1) { // statement rule index
      statements.push(child);
      console.log(`Found statement`);
    } else if (child.ruleIndex === 13) { // returnStatement rule index
      returnStmtCtx = child;
      console.log(`Found return statement`);
    }
  }
  
  console.log(`Declaring function: ${identifier}`);
...
CustomTrumplangVisitor.prototype.visitParameterList = function(ctx) {
  ...
  console.log(`ParameterList with ${ctx.getChildCount()} children`);
  for (let i = 0; i < ctx.getChildCount(); i++) {
    const child = ctx.getChild(i);
    const type = child.symbol ? `Token[${child.symbol.type}]:${child.getText()}` : 
                child.ruleIndex !== undefined ? `Rule[${child.ruleIndex}]` : 'Unknown';
...
CustomTrumplangVisitor.prototype.visitFunctionCall = function(ctx) {
  ...
  for (let i = 0; i < ctx.getChildCount(); i++) {
    ...
    if (child && child.symbol && child.symbol.type === 60) { // IDENTIFIER token type
      identifier = child.getText();
    } else if (child.ruleIndex === 10) { // argumentList rule index
      argListCtx = child;
    } else if (child && child.symbol && child.symbol.type === 59) { // VARIABLE token type directly in call
      argVars.push(child.getText());
    }
  }
  
  console.log(`Calling function: ${identifier}`);
  ...
  try {
    ...
    for (let i = 0; i < params.length; i++) {
      ...
      console.log(`Binding parameter ${param.name} to ${arg}`);
    ...
    console.log(`Function ${identifier} returned:`, result);
    return result;
  } finally {
...
CustomTrumplangVisitor.prototype.visitCondition = function(ctx) {
  ...
  if (comparisonCtx && exprCount === 2) {
    ...
    console.log(`Comparing: ${left} ${operator} ${right}`);
...
CustomTrumplangVisitor.prototype.visitArrayDeclaration = function(ctx) {
  ...
  console.log(`Declaring array: ${arrayName}`);
...
CustomTrumplangVisitor.prototype.visitArrayAccess = function(ctx) {
  ...
  console.log(`Trying to access array index ${index} of array with length ${array.value.length}`);
...
CustomTrumplangVisitor.prototype.visitDealField = function(ctx) {
  ...
  console.log(`Deal field with ${ctx.getChildCount()} children`); // Debug only
  ...
  for (let i = 0; i < ctx.getChildCount(); i++) {
    ...
    const childType = child.constructor ? child.constructor.name : 'unknown';
    const childText = child.getText ? child.getText() : 'no text';
    console.log(`  Field Child ${i}: ${childType} - "${childText}"`);
    ...
    } else if (child && child.symbol && child.symbol.type === 59) { // VARIABLE token type
  ...
  if (fieldType === 'DEAL') {
    ...
    console.log('Creating manual nested deal for field:', fieldName);
    ...
    if (inNestedDeal || nestedDealText === '') {
      console.log('Failed to parse nested deal structure');
    } else {
      console.log(`Found nested deal: ${nestedDealText}`);
      ...
      console.log(`Parsed ${fieldTexts.length} nested fields:`, fieldTexts);
      ...
      for (const fieldText of fieldTexts) {
        ...
        if (parts.length === 2) {
          ...
          if (lastSpace !== -1) {
            ...
            console.log(`  Nested field ${currentName} of type ${currentType} with value: ${currentValue}`);
      ...
      console.log(`Created nested deal with ${fields.length} fields:`, nestedDeal);
  ...
  for (let i = 0; i < ctx.getChildCount(); i++) {
    ...
    if (child.constructor && child.constructor.name === 'ExpressionContext') {
      valueExprCtx = child;
      console.log(`  Found value expression`);
    }
  }
  ...
  if (valueExprCtx) {
    ...
    console.log(`  Field ${fieldName} has value: ${value}`);
...
CustomTrumplangVisitor.prototype.visitDealDeclaration = function(ctx) {
  ...
  console.log(`Deal declaration with ${ctx.getChildCount()} children`); // Debug only
  ...
  for (let i = 0; i < ctx.getChildCount(); i++) {
    ...
    const childType = child.constructor ? child.constructor.name : 'unknown';
    const childText = child.getText ? child.getText() : 'no text';
    console.log(`  Child ${i}: ${childType} - "${childText}"`);
    
    if (child.constructor && child.constructor.name === 'DealFieldContext') {
      console.log(`  Processing deal field at index ${i}`);
  ...
  for (const field of fields) {
    ...
    console.log(`  Adding field ${fieldName} with value:`, field.value);
    dealObj[fieldName] = { type: field.type, value: field.value };
  }
  
  console.log(`Created deal structure with ${fields.length} fields:`, dealObj);
  return dealObj;
};
...
CustomTrumplangVisitor.prototype.visitDealAccess = function(ctx) {
  ...
  console.log(`Deal access with ${ctx.getChildCount()} children`); // Debug only
  ...
  console.log(`Looking for deal ${dealName} with field ${fieldName}`);
  
  // Get the deal structure
  const deal = this.getVariable(dealName);
  console.log(`Deal variable:`, deal);
  ...
  console.log(`Available fields:`, Object.keys(deal.value));
  ...
  if (deal.value[fieldName] && deal.value[fieldName].type === 'DEAL') {
    ...
    console.log(`Full access text: ${fullText}`);
    ...
    if (followCount > 1 || variables.length > 2) {
      ...
      if (fieldName === 'ADDRESS!' && variables.length > 2) {
        ...
        if (nestedDeal && nestedDeal[nestedFieldName]) {
          ...
          console.log(`Accessing nested field ${nestedFieldName} in ${fieldName} = ${result}`);
        } else {
          throw new Error(`THE NESTED DEAL ${fieldName} DOESN'T HAVE THIS TERM: ${nestedFieldName}. WORST DEAL EVER!`);
        }
      }
...
CustomTrumplangVisitor.prototype.visitFactorContext = function(ctx) {
  // Reuse the existing visitFactor method
  return this.visitFactor(ctx);
};
